...
const SPRAY_BURST = [
  { size: "h-10 w-10", color: "bg-vp-green/50", delay: 0, x: -6, y: -4 },
  { size: "h-12 w-12", color: "bg-vp-blue/45", delay: 40, x: 5, y: 3 },
  { size: "h-14 w-14", color: "bg-vp-green/40", delay: 80, x: -3, y: 6 },
  { size: "h-16 w-16", color: "bg-vp-blue/35", delay: 120, x: 4, y: -5 },
  { size: "h-20 w-20", color: "bg-vp-green/30", delay: 160, x: -5, y: -2 },
  { size: "h-24 w-24", color: "bg-vp-blue/25", delay: 200, x: 3, y: 4 },
  { size: "h-28 w-28", color: "bg-vp-green/25", delay: 240, x: -2, y: -6 },
];
...
export function ProductBlock() {
  return (
    <section className="relative z-10 mx-auto w-full max-w-[420px] px-5">
      <div className="relative mx-auto flex h-[195px] w-full items-center justify-center">
        <div className="absolute h-36 w-36 rounded-full bg-vp-green opacity-35 blur-2xl" />
        ...
        <Image
          ...
          height={945}
          priority
          className="vp-reveal relative z-10 h-auto max-h-[175px] w-auto max-w-[125px] object-contain"
        />
      </div>
      ...
      <ul className="mt-1 grid grid-cols-3 gap-2 text-center">
        {BULLETS.map(({ texto, Icon }) => (
          <li key={texto} className="flex flex-col items-center gap-1">
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-vp-green/50 text-vp-green">
